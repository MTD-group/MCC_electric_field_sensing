...
for calc in calc_list: #loop through all the modes in calc_list
	...
	for mod in mod_list: #loop through all the modulated structures for one mode
		...
		try:
                ...
                job_template = "job_b1027_temp_g.sh" # name of job_script template
                job_run = mod_dir+"job_b1027_"+prefix+'_'+str(calc)+'_'+str(mod)+"_g.sh" #name of the job script	
                imp_template = "./template_g.imp" # name of template input file
                imp_gbw = mod_dir +prefix+'_'+str(calc)+'_'+str(mod)+".gbw"  # name of wavefunction file if restarting a calcualtions
                imp_run = mod_dir +prefix+'_'+str(calc)+'_'+str(mod)+"_g.imp" #name of output file for the calculation
                imp_out = mod_dir +prefix+'_'+str(calc)+'_'+str(mod)+"_g.out" #name of output file for the calculation
		...
		# add the file name to the job template
                ...
                t3.insert( 1, "#SBATCH  --job-name=\"CuPc_HF_g_"+str(calc)+str(mod)+"\" ")		
